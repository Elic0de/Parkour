...
public class ListProcessor implements Consumer<Command.CommandInput> {
    private Function<String, List<RecordIds>> listSupplier;
    private String config;
    private boolean history;
    private boolean hasTarget;

    public ListProcessor(Function<String, List<RecordIds>> listSupplier, String config, boolean history, boolean hasTarget) {
    ...
    public void accept(Command.CommandInput input) {
        ...
        final List<RecordIds> punishments = listSupplier.apply(target);
...
/*MessageManager.sendMessage(input.getSender(), config + ".NoEntries",
        ...
        final Iterator<RecordIds> punishmentIterator = punishments.iterator();
        while (punishmentIterator.hasNext()){
            final RecordIds punishment = punishmentIterator.next();
        ...
        for (int i = (page - 1) * 5; i < page * 5 && punishments.size() > i; i++) {
            RecordIds punishment = punishments.get(i);
